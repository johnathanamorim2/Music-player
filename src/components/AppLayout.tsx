...
const useUserLibrary = (userId: string | undefined) => {
...
export const AppLayout = () => {
  ...
  // Dados da Biblioteca e Playlists
  ...
  }, [library, setPlaylist, isIndexPage]);

  // Se sair da página inicial, limpa os resultados da busca
  useEffect(() => {
    if (!isIndexPage) {
      setSearchResults([]);
      setCurrentTab("library"); // Volta para a aba de biblioteca ao sair
    }
  }, [isIndexPage]);
  ...
  const handleSearch = async (query: string) => {
    ...
    // Se a busca for iniciada em outra página, redireciona para a página inicial
    if (!isIndexPage) {
      navigate('/');
    }
    ...
    <div className="bg-gray-900 text-white min-h-screen">
      <div className="container mx-auto px-4 py-8 pb-32">
        ...
        <main className="mt-12">
          {/* Navegação Principal (Abas + Links) */}
          <MainNavigation 
            currentTab={currentTab} 
            onTabChange={setCurrentTab} 
          />

          {/* Se estiver na página inicial, exibe as abas de Busca/Biblioteca */}
          {isIndexPage ? (
            ...
            // Se não estiver na página inicial, renderiza o conteúdo da rota atual
            <Outlet />
          )}
        </main>
      </div>
      ...
      {/* Diálogos (Mantidos no Layout para acesso global) */}
